package com.example.landofmoviz.view.activity.see_all
...
import com.example.landofmoviz.databinding.ActivitySeeAllBinding
import com.example.landofmoviz.domain.model.*
import com.example.landofmoviz.utils.Constants
import com.example.landofmoviz.utils.IntentType
...
class SeeAllActivity : BaseActivity<ActivitySeeAllBinding>(R.layout.activity_see_all) {
    ...
    private fun getIntentExtras() {
        intent.apply {
            ...
            list = when (intentType) {
                IntentType.VIDEOS, IntentType.IMAGES, IntentType.CAST, IntentType.PERSON_CREDITS, IntentType.RECOMMENDATIONS -> getParcelableArrayListExtra<Parcelable>(
                    Constants.LIST
                ) as List<Any>
                else -> null
            }
            ...
            title = getStringExtra(Constants.TITLE) +
                    if (intentType == IntentType.GENRE) {
                        " " + if (mediaType == MediaType.MOVIE) getString(R.string.title_movies) else getString(
    ...
    private fun getList() {
        binding.recyclerView.adapter = when (intentType) {
            IntentType.VIDEOS -> VideoAdapter(true) {
                //    playYouTubeVideo(it)

            }.apply { submitList(list as List<Video>) }
            IntentType.CAST -> PersonAdapter(
                isGrid = true,
                isCast = true
            ).apply { submitList(list as List<Person>) }
            IntentType.IMAGES -> ImageAdapter(
                mediaType == MediaType.PERSON,
                true
            ).apply { submitList(list as List<Image>) }
            IntentType.PERSON_CREDITS -> when (mediaType) {
                MediaType.MOVIE -> MovieAdapter(isGrid = true, isCredits = true).apply {
                    submitList(
                        list as List<Movie>
                    )
                }
                MediaType.TV -> TvAdapter(
                    isGrid = true,
                    isCredits = true
                ).apply { submitList(list as List<Tv>) }
